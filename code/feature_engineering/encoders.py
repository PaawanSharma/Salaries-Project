...
# train = read_csv("data/train_features.csv", header=0)
# train["salary"] = read_csv("data/train_salaries.csv", header=0)["salary"]
# train.set_index("jobId", inplace=True)
# train = train[train["salary"] > 0]

# from numpy import mean

# oe = Ordinal_Encoder(mean, "salary")
# oe.fit(train)
# cm = oe.correlation_matrix(train)
...
# de = Dummy_Encoder(exclude=["companyId"])
# de.fit(train)
# rcm = de.reordered_correlation_matrix(train, reordering)

# de.fit(train, ["companyId"])
# my_tran = de.transform(train)
# my_tran.drop(["companyId"], axis=1, inplace=True)
# pd_tran = pd.get_dummies(train.drop(["companyId"], axis=1), drop_first=True)
